...
Production-grade pipeline to:
  1) Fit a reference PCA model on an LD-pruned SNP panel (VCF/BCF input)
  2) Save artifacts (variant means/stds, loadings, eigenvalues)
  3) Compute per-population centroids + shrinkage covariances in PC space
  4) Project new samples onto the reference PCs
  5) Assign ancestry via Mahalanobis (DR^2) with chi-square thresholds

Design goals:
- Deterministic, robust, and readable
- Strict site identity by default (chr,pos,ref,alt); optional allele swap/strand-flip with safeguards
- Handles chr vs no-chr prefixes transparently
- Built-in long-range LD masks (hg38) + user-provided BED
- Shrinkage covariance (LedoitWolf) for stable DR^2 inverses
- Clear, structured outputs suitable for downstream workflows

Dependencies:
  cyvcf2, numpy, pandas, scikit-learn, scipy

CLI overview:
  FIT REFERENCE MODEL
    ancestry_pca.py fit-ref \
      --vcf-pattern "/data/ALL.chr{chr}.vcf.gz" \
      --labels labels.tsv \
      --sites pca_sites.b38.tsv \
      --out pca_model/ \
      --pcs 12 \
      [--exclude-bed masks.bed] [--exclude-long-range] [--exclude-mhc] \
      [--allow-allele-swap] [--allow-strand-flip] [--impute-info-key DR2 --impute-min 0.8]

  PROJECT SAMPLES ONTO REF PCs
    ancestry_pca.py project \
      --vcf-pattern "/data/study.chr{chr}.vcf.gz" \
      --model pca_model/ \
      --out scores/ [--samples-keep keep.txt]

  ASSIGN ANCESTRY BY MAHALANOBIS
    ancestry_pca.py assign \
      --scores scores/query_scores.csv \
      --pop-params pca_model/pop_params.npz \
      --out assignments.csv [--k 12] [--p95 0.95 --p99 0.99]

Outputs (fit-ref):
  pca_model/
    ├── pca_sites.b38.tsv            (exact variant order used)
    ├── ref_means.npy                (float32; shape M,)   # mean dosage = 2p
    ├── ref_stds.npy                 (float32; shape M,)   # sqrt(2p(1-p))
    ├── loadings.npy                 (float32; shape M×K)  # variant loadings
    ├── explained_variance.npy       (float32; shape K,)
    ├── explained_variance_ratio.npy (float32; shape K,)
    ├── ref_scores.csv               (sample,super_pop,PC1..PCK) for reference
    └── pop_params.npz               (centroids, inverse covariances, pops, K)

Outputs (project):
  scores/
    └── query_scores.csv             (sample, PC1..PCK)

Outputs (assign):
  assignments.csv                    (label, best_pop, best_d2, second_pop, second_d2, margin, …)

Notes:
- This script computes PCA on reference samples only. Query samples are projected.
- DR^2 here is squared Mahalanobis distance in K-dimensional PC space (unbounded; ~χ^2_K).
- The “DR2 ≥ 0.8” you see in imputation pipelines is unrelated; that’s an imputation r^2 metric.
"""
